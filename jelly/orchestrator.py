...
def run_task(requirements_path: str, project_dir: str) -> dict:
    ...
    4. Run tests; if failures, feed errors back to Programmer (max 3 iterations).
    5. Write final code and test files to project_dir.
    ...
    # Step 1: Generate tests (separate session, requirements only, runs ONCE)
    console.print("\n[bold cyan]Step 1:[/] Generating tests from requirements...")
    test_designer = TestDesigner(config)
    test_files = test_designer.generate_tests(requirements, signatures)
    ...
    console.print("\n[bold cyan]Step 3:[/] Testing and iterating...\n")
    executor = TestExecutor(config)
    results: dict = {}

    for i in range(1, config.max_fix_iterations + 1):
        results = executor.run(code_files, test_files)
    ...
    console.print(f"\n[bold cyan]Step 4:[/] Writing output to [bold]{project_dir}[/]")
